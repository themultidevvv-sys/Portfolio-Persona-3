...
import menuVideo from "./assets/MainMenu.mp4";
import menuMusic from "./assets/MainMenu.mp3";
...
function MenuScreen({ mediaMuted, sfxMuted }) {
	...
	return (
		<div id="menu-screen">
			...
			<BackgroundMusic src={menuMusic} muted={mediaMuted} />
		...
		<AnimatePresence mode="wait">
			<Routes location={location} key={location.pathname}>
				...
				<Route
					...
					element={
						<PageTransition>
							<ResumePage mediaMuted={mediaMuted} sfxMuted={sfxMuted} />
						</PageTransition>
					}
				/>
				<Route
...
export default function App() {
	...
	const mediaMuted = musicMuted || soundMuted;
	const sfxMuted = soundMuted;

	return (
		<>
			<MediaControls
				mediaMuted={mediaMuted}
				onToggleMedia={() => setMusicMuted((value) => !value)}
				soundMuted={soundMuted}
				onToggleSound={() => setSoundMuted((value) => !value)}
			/>
			<AnimatedRoutes mediaMuted={mediaMuted} sfxMuted={sfxMuted} />
		</>
	);
}
